...
`scan.sh` fetches the latest rules from `raxITlabs/agent-security-review` (`main`), caches them locally, and falls back to the bundled copy in `references/rules-vendored/` if GitHub is unreachable. It writes ast-grep's JSON findings to stdout.
...
Full catalog: https://github.com/raxITlabs/agent-security-review/blob/main/docs/RULES.md
